...
# Path absolut berdasarkan lokasi file ini (kalender/calender.py)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_FILENAME = os.path.join(BASE_DIR, "kalender_event1.csv")
...
def generate_kalender():
    """Generate kalender event dan simpan ke CSV."""
    ...
    custom_seasons_data = []
    for start, end, name, tipe in CUSTOM_SEASONS:
        custom_seasons_data.extend(expand_date_range(start, end, name, tipe))

    df_custom_seasons = pd.DataFrame(custom_seasons_data)
    if not df_custom_seasons.empty:
        df_custom_seasons["Tanggal"] = pd.to_datetime(
            df_custom_seasons["Tanggal"]
        ).dt.date
    ...
    # Prioritaskan: 'Libur Nasional' menang di atas tipe lainnya
    ...
    try:
        ...
        print(f"File disimpan ke: {OUTPUT_FILENAME}")
        print("=" * 50)
        print(df_final.head())
    except Exception as e:
...
def load_kalender() -> pd.DataFrame:
    """
    Load kalender event dari CSV.
    Jika CSV belum ada, generate otomatis terlebih dahulu.
    Returns: DataFrame kalender, atau DataFrame kosong jika gagal.
    """
    if not os.path.exists(OUTPUT_FILENAME):
        print(f"File '{OUTPUT_FILENAME}' belum ada. Generating otomatis...")
        generate_kalender()
    ...
    try:
        ...
        print(f"File kalender tidak ditemukan: '{OUTPUT_FILENAME}'")
        return pd.DataFrame(columns=["Tanggal", "Nama_Event", "Tipe_Event"])
    except Exception as e:
        print(f"Error membaca kalender: {e}")
        return pd.DataFrame(columns=["Tanggal", "Nama_Event", "Tipe_Event"])
